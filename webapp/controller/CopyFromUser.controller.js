sap.ui.define([
    ...
    var CopyFromUserController = BaseController.extend("massiveusermgmt.controller.CopyFromUser", {
        ...
        getUserWorkCenters: function () {
            ...
            try {
                USER_BO = that.tableCopyFromUserUsersSrc.getContextByIndex(that.tableCopyFromUserUsersSrc.getSelectedIndex()).getObject().HANDLE;
            } catch (err) {
                that.getWorkCenters();
                that.selectedWorkCenters.length = 0;
            }

            var transaction = "8800/TRANSACTION/MASSIVE_CERTIFICATION/getWorkCentersByUser";
        ...
        onSave: function() {
            ...
            for (var c in that.modelCopyFromUserWorkCenters.getProperty("/")) {
                if (that.modelCopyFromUserWorkCenters.getProperty("/")[c].SELECTED) that.selectedWorkCenters.push(that.modelCopyFromUserWorkCenters.getProperty("/")[c].HANDLE);
            }

            that.selectedUsersDest.length = 0;

            for (var d in that.modelCopyFromUserUsersDest.getProperty("/")) {
                if (that.modelCopyFromUserUsersDest.getProperty("/")[d].SELECTED) that.selectedUsersDest.push(that.modelCopyFromUserUsersDest.getProperty("/")[d].HANDLE);
            }

            that.selectedUserGroupsDest.length = 0;

            for (var g in that.modelCopyFromUserUsersDest.getProperty("/")) {
                if (that.modelCopyFromUserUsersDest.getProperty("/")[g].SELECTED) that.selectedUserGroupsDest.push(that.modelCopyFromUserUsersDest.getProperty("/")[g].HANDLE);
            }
